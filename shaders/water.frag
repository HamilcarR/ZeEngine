...
uniform vec3 camPos;
const float shineDamping=1.5F;
const float reflectivity=0.0001F;
...
void main(void){
 ...
 float disx=(texture2D(DUDV,tex  + mod(time,att))*2. - 1.).r*att; 
 float disy=(texture2D(DUDV,tex  - mod(time,att))*2. - 1.).g*att; 
...
vec2 unn = vec2(cos(disy*time),sin(disy*time)); 
vec3 unitNormal = vec3(un) ;
vec3 total=vec3(0.0);
...
float specular= max(dot(reflectedLight,posCamera), 0.F);
float damping= pow(specular,shineDamping);
finalSpec += damping*lightColor[i]*reflectivity;

float brightness=clamp(dot(un,unitToLight),0.0,1.0);
...
total+=diff/attenuationFact;
 ...
 //color= vec4(un,1.);
 color=mix(reflection ,refraction, fresnel )+vec4(0.,0.,0.05,1.);//+vec4(finalSpec,1.) ;
